...
interface IVerifyingClaimerV2 {
    function claimedAccount(address account) external view returns (bool);
    function claimedZkId(bytes32 zkId) external view returns (bool);
}

contract VerifyingClaimerV2_1 is Ownable {
    ...
    IVerifyingClaimerV2 public immutable verifyingClaimer;
    ...
    constructor(address _claimer, address _vault, address _validator, address _proofVerifier, address _token, bytes32 _root, uint256 _endTimestamp) Ownable(msg.sender) {
        verifyingClaimer = IVerifyingClaimerV2(_claimer);
    ...
    function claim(
        ...
        if (claimedAccount[_account] || verifyingClaimer.claimedAccount(_account)) {
            revert ClaimedAccount(_account);
        }
        if (_zkId != 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470) {
            if (claimedZkId[_zkId] || verifyingClaimer.claimedZkId(_zkId)) {
